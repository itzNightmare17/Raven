...
from core import LOGS, raven_bot, HNDLR, rm
...
**OwnerID** - `{raven_bot.uid}`

**Message Forwards** - {udB.get_key("PMBOT")}

**Raven [v{version}](https://github.com/itzNightmare17/Raven), powered by @TeamRaven**
"""
...
async def own(event):
    ...
    msg = message.format(
        mention=event.sender.mention, me=inline_mention(raven_bot.me)
    )
    if custom:
        msg += "\n\n• Powered by **@TeamRaven**"
    await event.edit(
        msg,
...
async def raven_handler(event):
    ...
    if event.sender_id not in fullsudos():
        ok = ""
        me = inline_mention(raven_bot.me)
        ...
        if _starts := udB.get_key("STARTMSG"):
            ...
            msg = f"Hey there {mention}, this is Raven Assistant of {me}!\n\n{ok}"
        await event.reply(
...
async def setlang(event):
    ...
    travd = [
        Button.inline(
            f"{languages[rav]['name']} [{rav.lower()}]",
            data=f"set_{rav}",
        )
        for rav in languages
    ]
    buttons = list(zip(travd[::2], travd[1::2]))
    if len(travd) % 2 == 1:
        buttons.append((travd[-1],))
