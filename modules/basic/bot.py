...
from .. import HOSTED_ON, get_string, udB, raven_cmd


@raven_cmd(pattern="ping$")
...
async def logs_func(event):
    ...
    if arg == "open":
        ...
        with open(file_path, "r") as f:
            code = f.read()[-2500:]
        file = await Carbon(
            file_name="raven-logs",
            code=code,
            backgroundColor=get_random_color(),
        )
        if isinstance(file, dict):
            return await event.eor(f"`{file}`")
        return await event.eor("**Raven Logs.**", file=file)
    elif arg == "graph":
        ...
        return await event.eor(f'[Raven Logs]({page["url"]})', link_preview=True)
    await event.eor(file=file_path)


@raven_cmd(pattern="restart$")
