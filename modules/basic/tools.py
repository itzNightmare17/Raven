...
from .. import LOGS, get_string, humanbytes, raven_cmd


@raven_cmd(pattern="tr( (.*)|$)", manager=True)
...
@raven_cmd(pattern="q( (.*)|$)", manager=True, allow_pm=True)
async def q_func(event):
    ...
    message = await reply.reply("Quotly by Raven", file=file)
    os.remove(file)
    await msg.delete()
    return message
...
async def invite_func(rav):
    xx = await rav.eor(get_string("com_1"))
    to_add_users = rav.pattern_match.group(1).strip()
    add_chat_user = not rav.is_channel and rav.is_group
    ...
    for user_id in users:
        try:
            if add_chat_user:
                kwargs = {
                    "chat_id": rav.chat_id,
                    "user_id": await rav.client.parse_id(user_id),
                    "fwd_limit": 1000000,
                }
            else:
                kwargs = {
                    "channel": rav.chat_id,
                    "users": [await rav.client.parse_id(user_id)],
                }
            await rav.client(request(**kwargs))
            await xx.edit(f"Successfully invited `{user_id}` to `{rav.chat_id}`")
