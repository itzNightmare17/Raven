...
@raven_cmd(pattern="bash", fullsudo=True, only_devs=True)
async def bash_func(event):
    ...
    if len(OUT) > 4096:
        rave = err + out
        with BytesIO(str.encode(rave)) as out_file:
...
@raven_cmd(pattern="eval", fullsudo=True, only_devs=True)
async def eval_func(event):
    ...
    if (
        ...
        and not event.out
        and event.sender_id != raven_bot.uid
    ):
