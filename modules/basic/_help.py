...
from .. import HNDLR, LOGS, asst, get_string, inline_pic, udB, raven_bot, raven_cmd
...
def get_from_funcs(mod, name):
    handlers = list(map(lambda d: d[0], raven_bot.list_event_handlers()))
    funcs = list(
        filter(
            lambda d: (
    ...
    if not module:
        if event.client._bot:
            return await event.reply(
                get_string("inline_4").format(raven_bot.full_name, len(PLUGINS)),
                file=inline_pic(),
                buttons=get_help_buttons(),
                link_preview=False,
            )
        await event.delete()
        result = await event.client.inline_query(asst.me.username, "rave")
...
@in_pattern("rave", owner=True)
async def inline_handler(event):
    text = get_string("inline_4").format(raven_bot.full_name, len(PLUGINS))
    if inline_pic():
        ...
        result = await event.builder.article(
            title="Raven Help Menu", text=text, buttons=get_help_buttons()
        )
    await event.answer([result], private=True, cache_time=300, gallery=True)


@callback(re.compile("uh_(.*)"), owner=True)
async def help_func(rav):
    key, count = rav.data_match.group(1).decode("utf-8").split("_")
    plugs = filter_modules(key)
    if key == "vcbot" and not plugs:
        return await rav.answer(get_string("help_12"), alert=True)
    elif key == "addons" and not plugs:
        return await rav.answer(get_string("help_13").format(HNDLR), alert=True)
    if "|" in count:
        _, count = count.split("|")
    ...
    text = get_string(_strings.get(key, "")).format(raven_bot.full_name, len(plugs))
    await rav.edit(text, buttons=page_num(count, key), link_preview=False)
