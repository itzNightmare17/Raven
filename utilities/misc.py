...
async def randomchannel(
    tochat, channel, range1, range2, caption=None, client=raven_bot
):
...
async def allcmds(event, telegraph):
    ...
    with rm.get("graph", helper=True, dispose=True) as mod:
        ...
        t = telegraph.create_page(title="Raven All Cmds", content=[txt])
    await eor(event, f"All Raven Cmds : [Click Here]({t['url']})", link_preview=False)
